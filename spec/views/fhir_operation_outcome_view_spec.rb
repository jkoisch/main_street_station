...
describe 'FHIR OperationOutcome View' do
  it 'should produce valid JSON' do
    @operation_outcome = YAML.load(File.read('spec/support-files/operation-outcome.yaml'))
    response = render template: 'fhir/fhir_base/operation_outcome', formats: :json
    expected = File.read('spec/support-files/fhir-operation-outcome.json')
  ...
  it 'should produce valid XML' do
    ...
    expected = File.read('spec/support-files/fhir-operation-outcome.xml')
    expect(response).to eq(expected)
  end
end
