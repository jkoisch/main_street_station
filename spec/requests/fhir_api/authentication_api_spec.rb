...
describe 'Authentication Request FHIR API', type: :request do
  ...
  describe '- user verification' do
    describe ' - internal' do
      ...
      it 'should allow a user to log in' do
        post '/api_session', params: {user_name: user.email, password: valid_pw}
        expect(response.status).to eq 200
      end

      it 'should reject a user that does not exist' do
        bad_email = user.email.tr('aei', '123')
        post '/login', params: {user_name: bad_email, password: valid_pw}
        expect(response.status).to eq 401
      end

      it 'should reject a user when password invalid' do
        post '/login', params: {user_name: user.email, password: '123temp'}
        expect(response.status).to eq 401
      end
  ...
  describe '- token validation' do
    ...
    it 'should fail with an invalid token' do
      get '/fhir/Observation',
          headers: {'HTTP_AUTHORIZATION' => ActionController::HttpAuthentication::Token.encode_credentials('123456')}
      expect(response.status).to eq 401
    end
    ...
    it 'should pass with a valid token' do
      ...
      get '/fhir/Observation',
          headers: {'HTTP_AUTHORIZATION' => ActionController::HttpAuthentication::Token.encode_credentials(current_token.authentication_token)}
      expect(response.status).to eq 200
    end
    ...
    it "should return 'Not Authorized' with an expired token" do
      ...
      user_token.save!
      get '/fhir/Observation',
          headers: {'HTTP_AUTHORIZATION' => ActionController::HttpAuthentication::Token.encode_credentials(user_token.authentication_token)}
      expect(response.status).to eq 401
    end
  end
